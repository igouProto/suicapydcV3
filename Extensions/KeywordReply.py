...
class KeywordReply(commands.Cog):
    ...
    async def on_ready(self):
        ...
        for guild in self.bot.guilds:
            try:
                ...
                source.close()
                log.info("Loaded keyword pairs for guild {} ({})".format(guild.name, guild.id))
            except Exception as e:
                log.error("Failed to load keyword pairs for guild {} ({}): {}".format(guild.name, guild.id, e))
    ...
    async def on_message(self, message: discord.Message):
        ...
        if message.content in self.public_dictionary.keys():
            ...
            if message.content in self.guild_dictionaries[message.guild.id].keys():
                await message.channel.send(self.guild_dictionaries[message.guild.id][message.content])
                ...
                try:
                    with open('Assets/keywords-{}.txt'.format(ctx.guild.id), 'w') as source:
                        for key in self.guild_dictionaries[ctx.guild.id].keys():
                            source.write("{}:{}".format(key, self.guild_dictionaries[ctx.guild.id][key]))
                    source.close()
                except Exception as e:
                    log.error("Failed to write to keywords-{}.txt: {}".format(ctx.guild.id, e))
    ...
    async def _backup_guild_kw(self, ctx: commands.Context):
        if ctx.guild.id in self.guild_dictionaries.keys():
            ...
            await ctx.send(file=discord.File('Assets/keywords-{}.txt'.format(ctx.guild.id)))
        else:
            await ctx.send(Messages.KEYWORD_GUILD_NOT_FOUND.format(ctx.prefix))
    ...
    async def _upload_guild_kw(self, ctx: commands.Context):
        if len(ctx.message.attachments) > 0:
            ...
            if attachment.filename == 'keywords.txt':
                ...
                # load every guild's keyword-reply pairs from the file
                try:
                    with open('Assets/keywords-{}.txt'.format(ctx.guild.id), 'r') as source:
                        self.guild_dictionaries[ctx.guild.id] = {}
                        for line in source:
                            (keyword, reply) = line.split(':', 1)
                            self.guild_dictionaries[ctx.guild.id][keyword] = reply
                    source.close()
                    log.info("Loaded keyword pairs for guild {} ({})".format(ctx.guild.name, ctx.guild.id))
                except Exception as e:
                    log.error("Failed to load keyword pairs for guild {} ({}): {}".format(ctx.guild.name, ctx.guild.id, e))
                log.info("Loaded {} keyword pairs for guild {}".format(len(self.guild_dictionaries[ctx.guild.id]), ctx.guild.name))
